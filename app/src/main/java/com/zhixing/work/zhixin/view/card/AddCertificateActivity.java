...
//                        .isDialog(true)//是否显示为对话框样式
    ...
    private void upCertificate(RequestBody body) {
        OkUtils.getInstances().httpPut(body, context, RequestConstant.ADD_CERTIFICATE, JavaParamsUtils.getInstances().upCertificate(), new TypeToken<EntityObject<Boolean>>() {
    ...
    private void deleteData(RequestBody body) {
        OkUtils.getInstances().httpDelete( context, RequestConstant.ADD_CERTIFICATE + "?Id=" + bean.getId(), JavaParamsUtils.getInstances().deleteWork(bean.getId() + ""), new TypeToken<EntityObject<Boolean>>() {
        }.getType(), new ResultCallBackListener<Boolean>() {
            @Override
